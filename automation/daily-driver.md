...
- **manifest の Write は「最後の必須アクション」。何があっても manifest を書いてから応答を終えること。** 入力の全トピックぶんのエントリを含む manifest を書かずに応答を終えるのは重大違反（シェルが「manifest 欠落＝全記事 error 退避」と判定し、生成した良質記事もすべて公開されず捨てられる）。ある記事の画像が最後まで用意できない等で完成しなくても、**その記事を `passed=false` / `note="error: <理由>"` として記録し、manifest 自体は必ず書く**（完成した他記事は公開できる＝0 公開を避ける）。
- **画像生成・R2 アップロードはすべて同期的に最後まで完了させる。バックグラウンド実行や「完了通知を待つ／後で確認する」形で応答を終えてはならない。** headless では通知でターンが再開しないため、待ち状態で応答を終えると未完のまま終了扱いになる。figure の再生成が必要な場合もその場で完了まで回し、全記事の全画像が R2 に上がったことを確認してから manifest を書く。どうしても完了できない画像がある記事は上記のとおり `passed=false` にして先へ進む（止まらない・待たない）。
