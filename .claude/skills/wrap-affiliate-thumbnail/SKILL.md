...
user-invocable: true
argument-hint: "<output-dir> <prefix> <title> <subtitle> [--category <text>] [--logo <url-or-path>] [--accent navy|light|teal|blue|orange] [--n 2]"
---
...
- **Input**:
  ...
  - `--accent` (optional): `navy` / `light` / `teal` / `blue` / `orange` (default=navy)
  - `--n` (optional): 並列数 (default=2)。コスト/時間優先のため 2 が標準。品質バラつきが心配な記事のみ 4 に上げる
...
## 製品 → accent-theme の推奨マッピング (2026-05-23 更新)

| product | 必須 accent-theme | 理由 |
|---|---|---|
| `brightdata` | `navy` | Bright Data 公式の青系コーポレートカラー (現状維持) |
| `1password` | `blue` | 1Password 公式ブランドブルー (#0572EC 系)。 navy より明るく彩度高めだが、依然として落ち着いた青で誠実トーンを保つ |
| `nextengine` | `orange` | Next Engine 公式のオレンジ系コーポレートカラー (#EA580C 系) を踏襲。彩度を抑えてビジネス白書トーンを維持 |

**重要 (2026-05-23 改修)**:
- 3 製品のサムネが視覚的に明確に区別できるよう、各製品の公式ブランドカラーをベースにテーマを再設計した。
- 「同一プロダクトの記事群は同じ accent-theme を必ず使う」 = シリーズ感
- 「異なるプロダクト間でテーマを混ぜない」 = 1Password 記事に orange を使う等は禁止
- 過去に 1password=teal、nextengine=light で運用していた時期があるが、視覚的区別が薄かったため新マッピングに移行
- ユーザーフィードバック (2026-05-23): 「1Password と Next Engine と Bright Data でサムネのトーンは明確に区別すること。Bright Data の navy は維持」
...
```bash
# product → accent のマッピング (固定)
#   brightdata → navy
#   1password  → blue
#   nextengine → orange
ACCENT="navy"  # product に応じて (上記マッピング参照、変更禁止)
