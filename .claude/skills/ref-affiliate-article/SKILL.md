...
6. **クラスターバランスの確認 (v3)**:
   - `subtopics.json` は候補カタログであり「網羅済みチェックリスト」ではない。採用状態は `existing-slugs.json` との差分 (Step 1.5 の `unused-cells.json`) が唯一の真実
   - 現状は brightdata に偏っている (BD 多 / NE 僅少)。**新規テーマは未使用セルが多い product・category を優先**し、同一 category に 3 本以上ある product では別 category へ振る
   - pillar (土台記事: pricing / setup / 主要 vs-comp) が無い product・category を最優先で埋める

generator はこの結果を context JSON 経由で受け取り、(a) 未使用 slug を採用、(b) 関連記事候補を本文内リンクに織り込む。

---

## §3. 評価ルーブリック (採点 11 軸、満点 145 点)

`assign-affiliate-article-evaluator` の `eval-schema.json` (version 3) の `breakdown_keys` と同じキー名・配点。正規化後 `(score / 145) * 100 >= 90` で合格。

軸一覧: `seo`(20) / `readability`(15) / `intent_fit`(10) / `cv_readiness`(10) / `affiliate_fit`(15) / `factual_accuracy`(15) / `x_embed`(10) / `product_pitch`(15) / `duplication`(15) / `ja_en_parity`(10) / `link_health`(10)。

> v3 の追加: `intent_fit` (検索意図別の必須ブロック充足) と `cv_readiness` (読者の意思決定を進める成約力) を独立軸化。従来「7 軸 100 点 / Haiku 採点」だった記述は無効。採点は Opus (grep・構造) + Codex (factual_accuracy) + Bash (link_health)。
...
| 観点 | 配点 | 減点の目安 |
|---|---|---|
| CTA (`<InlineCTA />`) が **§7 の product 別 CTA 基準表**に適合 (個数・種別・同一 intent 重複なし) | 2 | 基準表逸脱 -2 / 合計 4 個以上 -3 |
...
| Disclosure 文言 (末尾「※本記事には PR を含みます」。冒頭開示はサイトが自動描画) が本文末尾にある | 2 | 完全に欠落 -2 |
| AF URL が config (`affiliate.ts`) の正規 URL と一致 (別製品・別 ID・公式サイト誤爆なし) | 2 | 別製品 AF 混入 -3 |
...
**evaluator (`assign-affiliate-article-evaluator`, `context: fork` で別 subagent / Opus + Codex) は以下を必ず実行**:
...
### §3.7 `ja_en_parity` (10 点) — ja/en 整合性 (直訳一致ではなく事実整合)

**方針転換 (v3)**: en は ja の逐語訳ではなく、英語圏の検索意図に合わせた独自構成を許容する。parity は「事実の一貫性」のみ必須とし、構成・見出し・FAQ・例示のロケール最適化差は減点しない。

| 観点 | 配点 | 減点の目安 |
|---|---|---|
| 数値・料金・URL・固有名詞・製品名が ja/en で完全一致 | 4 | 不一致 -4 (致命。片方だけ古い価格等) |
| 同一の一次情報 (公式ページ・脚注 URL) を参照 | 2 | 参照ソースが食い違う -2 |
| 通貨表記が ja=円メイン (USD 補記)、en=USD メイン (JPY 補記) | 2 | 通貨が片方しかない -2 |
| 主題・結論・対象読者が両言語で一致 (見出し順の一致は不要) | 2 | 主張がズレている -2 |

**nextengine の ja 単独記事**: en を生成しない運用では本軸を評価対象外とし満点 (10) 固定。`ja_en_parity.notes` に `"ja-only (nextengine): parity N/A"` と記録する。

### §3.8 `factual_accuracy` (15 点) — 事実・数値の正確性

数値・料金・件数・日付・固有情報は必ず公式一次情報と突合する。evaluator は Codex (`delegate-codex`) 経由で公式 URL を WebFetch し、抽出した各主張を verified / mismatch / unverified / fabricated に分類する。

| 状態 | スコア |
|---|---|
| 全 claim が verified (or unverified を明示的に断り書き) | 15 |
| unverified 2 件以内・mismatch ゼロ | 13-14 |
| mismatch 1 件 | 10 |
| mismatch 2 件以上 | 5 (threshold 割れ→不合格) |
| **fabricated 1 件以上** | **0 (致命・即不合格)** |

公式ソース: brightdata=`https://brightdata.com/pricing` / nextengine=`https://next-engine.net/price/` / 1password=`https://1password.com/jp/pricing`。X 引用は tweet ID が `x-search.json` の `inline_citations` に実在するか検証。捏造した数値・キャンペーン・引用は他軸の高得点で相殺できない (公開不可)。過去事故: NE 月額 3,000 円を 10,000 円と誤記 / 1Password 月払い基準を年払いと混同。

### §3.9 `link_health` (10 点) — リンク健全性

MDX 内の外部リンク・脚注 URL を HTTP HEAD (403/405/000 は GET フォールバック、`-L` でリダイレクト追従) で検証し、2xx 以外を減点。

| 状態 | 控除 |
|---|---|
| 全 URL 2xx | 0 (満点 10) |
| 4xx/5xx 1 件 | -3 |
| 4xx/5xx 2-3 件 | -6 (threshold 割れ→不合格) |
| 4xx/5xx 4 件以上 | -10 |
| 非 HTTPS リンク | -1 / 件 |

**アフィリエイトリンクは検証対象から除外する**。`affiliate.ts` の `AFFILIATE_LINK_HOSTS` (`get.brightdata.com` / `base.next-engine.org` / `1password.partnerlinks.io`) に一致する URL は curl せず、「config 記載の正規 AF URL と文字列一致するか」だけ確認する。理由: (1) AF リンクを大量 HEAD するとクリック計測やアンチボットを汚す、(2) 302 チェーンの先が動的で誤検知しやすい。AF URL が config と食い違っていたら `affiliate_fit` で減点。

### §3.10 `intent_fit` (10 点) — 検索意図適合

記事の検索意図タイプ (§4.1 の category→intent マッピング) に対し、必須ブロックが揃い、SERP ブリーフ (`serp-brief.json`: 上位タイトル・見出し・PAA) の主要論点をカバーしているか。

| 観点 | 配点 | 減点の目安 |
|---|---|---|
| 宣言 intent の必須ブロックが揃う (§4.2 表) | 4 | 必須ブロック欠落ごとに -2 |
| SERP ブリーフ上位の見出し論点を過半カバー | 3 | 主要論点の半分未満 -3 / ブリーフ無しなら減点しない |
| PAA / 関連質問を FAQ か H2 で回収 | 2 | 主要 PAA を無視 -2 |
| intent と CTA の検討段階が一致 (BOFU=購入導線明確 / TOFU=学習優先で押し売りなし) | 1 | 不一致 -1 |

### §3.11 `cv_readiness` (10 点) — 成約力 (意思決定支援の充足)

読者が「自分は導入すべきか」を判断できる材料が揃っているか。`affiliate_fit` (規約遵守) や `product_pitch` (製品の強み) とは別に、**読者の意思決定を前に進める要素**を採点。BOFU 記事 (pricing/vs-comp/migration) で重視。

| 観点 | 配点 | 減点の目安 |
|---|---|---|
| **「向いている人／向いていない人」**が代替案付きで整理 (BOFU 必須) | 3 | 欠落 -3 (BOFU) / -1 (TOFU) |
| **不安解消要素を 2 つ以上** (解約・返金・無料期間・日本語対応・データ移行・セキュリティ懸念から) | 3 | 0 個 -3 / 1 個 -1 |
| 意思決定を助ける具体要素 (料金シミュ / 比較決定表 / 導入工数 / ROI 目安) が intent に応じ最低 1 つ | 2 | 抽象論のみ -2 |
| 次の一手が明確 (無料で試す→何が起きる、相談→何を得る) | 2 | CTA 先の体験が不明 -2 |

**ROI・成果は保証しない**。「必ず◯倍」等は書かせず「一般的な目安」「弊社の実測レンジ」等に留める (虚偽の成果保証は `factual_accuracy` と `affiliate_fit` で致命)。
...
### §4.1 検索意図タイプと category マッピング

記事は 1 本 = 1 検索意図。`subtopics.json` の category から intent を決め、必須ブロックを満たす。

| category | intent | 読者の状態 | 主 CTA トーン |
|---|---|---|---|
| `pricing` | **Transactional** (BOFU) | 導入直前、費用を確定したい | 「無料で試す」を明確に |
| `vs-comp` | **Comparative** (BOFU) | 2〜3 製品で迷っている | 勝者提示 + 試用導線 |
| `migration` | **Comparative/How-to** (BOFU) | 乗り換えを検討中 | 移行手順 + 乗り換え導線 |
| `setup` `troubleshooting` | **How-to** (MOFU) | 使い始め・詰まり解消 | 補助的、押し売りしない |
| `feature-deep-dive` `security` | **Informational** (MOFU) | 機能・安全性を理解したい | 学習優先、末尾に軽く |
| `case-study` | **Informational/Comparative** | 効果イメージを掴みたい | 事例 → 相談/試用 |
| `faq` `update-news` | **Informational** (TOFU) | 疑問解消・最新把握 | 最小限 |

### §4.2 intent 別・必須ブロック

すべての intent 共通: リード (結論ファースト 150-200 字) / 本文 H2 3〜6 個 / 一次情報の脚注 / disclosure (末尾 PR 注記 + サイト側冒頭自動) / FAQ (frontmatter)。intent ごとに以下を**追加必須** (欠落は `intent_fit` / `cv_readiness` で減点):

- **Transactional (pricing)**: 料金体系を 1 枚で示す表 / **利用量・受注件数別の料金シミュレーション** / 隠れコスト・追加費用の明示 / 「向いている人・向いていない人」/ 不安解消 2 つ (無料期間・解約等)。
- **Comparative (vs-comp / migration)**: **比較決定表** (製品 × 評価軸、◯△× でなく一言根拠付き) / **ケース別の勝者** (「小規模なら A、SSO 必須なら B」) / (移行記事は) 手順・ダウンタイム・データ移行の扱い / 「向いている人・向いていない人」。
- **How-to (setup / troubleshooting)**: 前提条件 / **番号付き手順チェックリスト** / つまずきポイントと対処の早見表 / **ロールバック・やり直し手順** または検証方法。
- **Informational (feature / security / faq)**: 定義 → 仕組み → 実務での意味の順 / 図解 1 枚以上 / 誤解されやすい点の補足 / 関連する意思決定への橋渡し 1 文。

### §4.3 基本スケルトン (Informational 既定。他 intent は §4.2 ブロックを差し込む)

```
1. リード文 (150〜200 字 / 80〜120 words): 結論ファースト + 悩み提示 + 約束
2. H2-1: 背景・前提 / What is X (製品名にインライン AF リンク 1)
3. H2-2: 主題のコア (intent 別必須ブロックの中心。表・リスト必須)
4. H2-3: 実践 / 比較 / 手順 (<TweetCard /> 1 + <Figure /> 1)
   └ 本文 30〜50% 地点に <InlineCTA intent="primary" /> (§7 CTA 基準表に従う)
5. H2-4: 注意点 / Pros&Cons / 向き不向き
6. H2-5: (BD/NE のみ) 弊社支援 1〜2 文 + <InlineCTA intent="support" /> / (関連成立時) self-product CTA
7. H2-6: まとめ (再結論 + 次の一手。まとめ冒頭の製品名に AF リンク)
8. FAQ (frontmatter faq に格納。本文には書かない)
9. 末尾: 「※情報は YYYY-MM-DD 時点」+「※本記事には PR を含みます」
    (詳細ページ末尾の large CTA / 著者 / 関連記事はサイトが自動描画)
```

H2 を圧縮する場合も、宣言 intent の §4.2 必須ブロックは省略しない。
...
### §5.1 title / summary 生成ルール (CTR 最適化)

- **title (ja)**: 主要 KW を**前方**に置き、目安 28〜40 字 (メタタイトル切れ回避)。数字・便益・具体語を 1 つ入れる。**語尾パターンを機械化しない** (「〜完全ガイド 2026」を全記事に付けない。比較=「どっち/選び方」、料金=「いくら/料金表」、手順=「手順/やり方」など intent に合わせて変える)
- **title (en)**: 主要英語 KW を前方に。60 char 以内。US 検索の言い回し (例: "pricing" であって "price plans" ではない)
- **年号の扱い**: `slug` には年号を焼き込まない (URL は永続資産。`bright-data-pricing` とし `-2026` を付けない。既存で付いているものは踏襲)。title/summary の「2026 年版」は更新運用 (`updated` 更新 + refresh モード) 前提で可
- **summary (meta description)**: ja 120 字 / en 155 char 以内。主要 KW + 読者便益 + 次アクションの示唆を 1 文で。本文リードのコピペにしない
...
#### product 別 CTA 基準表 (本文中の `<InlineCTA />`。末尾 large は別で自動)

| product | inline `primary` | inline `support` | `self-product` | 本文中 合計 |
|---|---|---|---|---|
| **brightdata** | 1 (必須) | 0〜1 | 0〜1 (Tra-bell) | 1〜3 |
| **nextengine** | 1 (必須) | 0〜1 | 0〜1 (CataMap、楽天/Yahoo!/商品マスタ文脈時のみ) | 1〜3 |
| **1password** | 1 (必須) | **0** (support 文言を持たない) | **0** (基本出さない。§11) | 1 |

- **同一 intent の `<InlineCTA />` を 2 個以上置かない** (末尾 large 自動描画があるため primary は本文 1 個で十分)。
- 1password は support/self-product を持たないため、本文 inline CTA は primary 1 個 + 末尾 large 自動のみ。過剰 CTA は `affiliate_fit` で減点。
...
- リンク表記は素直な `[Bright Data](URL)` 形式。**`rel` 属性は MDX に書かない**。サイト側 (`mdx-components.tsx`) が AF ホスト (`affiliate.ts` の `AFFILIATE_LINK_HOSTS`) を判定して `rel="sponsored nofollow noopener noreferrer"` を自動付与する。手書きの `{:rel=...}` はレンダリングされず不要
...
- `wrap-affiliate-thumbnail` (cover/og) / `wrap-ai-image-detail-illustration` (figures) で生成、wrangler でアップロードまで自動化 (`run-affiliate-article` Step 8)
...
[run-affiliate-article]
  ├─ 既存記事 slug/title/tag 抽出 + MECE 検証 (未使用セル / 関連候補)
  ├─ SERP リサーチ (WebSearch: 上位タイトル・見出し・PAA → serp-brief.json)
  ├─ X 検索 (run-x-search)
  ├─ assign-affiliate-article-generator (ja/en。nextengine は ja 単独可)
  ├─ assign-affiliate-article-evaluator (Opus + Codex で factual_accuracy、90 点合格)
  ├─ 不合格 → feedback.md を generator に戻す (最大 3 周)
  ├─ wrap-affiliate-thumbnail (cover / og、business-headline)
  ├─ wrap-ai-image-detail-illustration (figures 2〜3 枚)
  ├─ wrangler R2 upload
  └─ src/lib/content/{ja,en}/media/<slug>.mdx 書き出し
```

評価ループ上限は 3 周。それでも 90 点未到達なら人間に渡す (assign-slide-generator と同じ流儀)。採点は 11 軸 145 点 (§3)。
