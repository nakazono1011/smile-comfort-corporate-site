...
1. `output_contract.ref_skill_md` (ref-affiliate-article SKILL.md)
2. `output_contract.voice_guide` (voice-guide.md)
3. `output_contract.product_ref` (product-<product>.md)
4. `output_contract.template` (article-template.mdx.tmpl)
...
1. `unused_cells_path` を読み、`product == 指定` のセルから topic に最も近いものを優先採用 (`slug_pattern` をそのまま流用)
2. 一致する未使用セルが無い場合のみ、`subtopics.json[product].categories[*].examples[].slug_pattern` を派生 or 新規生成
3. `existing_slugs_path` と衝突する場合は派生語 (年月・地域・観点) で必ずユニーク化 (例: `bright-data-pricing-2026` → `bright-data-pricing-volume-discount-2026`)
...
本文は ref-affiliate-article §4 のテンプレに従う:
- リード 150-200 字 (結論ファースト)
- 本文 H2 3-5 個 (各 H2 = 2-4 H3)
- 本文中央付近に `<InlineCTA product="<product>" intent="primary" />` を 1 個 (必須)
...
- **本文中の AF 製品名にインラインアフィリエイトリンクを 3〜5 箇所** (ja/en それぞれ独立カウント)。詳細ルール・URL マップ・配置原則は ref-affiliate-article §7.5 を参照。URL は product-`<product>`.md の「アフィリエイトリンク」を流用 (CTA と同 URL)。同一段落での 2 連続や、コードブロック・脚注・frontmatter への挿入は禁止
- `<TweetCard id="..." />` を 1-3 個
- `<Figure src="https://img.smile-comfort.com/<slug>/figure-01.webp" alt="..." caption="..." />` を 2-3 枚
- FAQ は frontmatter `faq` に 3-5 件入れる (本文 H2 として FAQ を書いてもよいが、frontmatter にも入れる)
...
ja の構造を踏襲しつつローカライズ。voice-guide.md §3 のルールに従う。

- title は ja のキーワードに対応する英語 SEO キーワード
- 文字数 **en 1,800〜2,500 words**
- 通貨は USD メイン (¥補記)
- 数値・URL・固有名詞は ja と完全一致
- H2 数を ja と揃える
- **本文中のインラインアフィリエイトリンクも ja と同等の 3〜5 箇所** (en 側で個別に英文の自然な誘導点に挿入)。URL は同じ AF URL を使う
...
```json
{
  ...
  "image_briefs": {
    "cover": {
      "prompt": "<thumbnail prompt for wrap-thumbnails>",
      "filename": "cover.webp",
