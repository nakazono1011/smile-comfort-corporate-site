...
import { EXTERNAL_LINKS } from "@/config/company";

const products = [
  {
    name: "CataMap",
    tagline: "AIが考える、売れるタグ。",
    description:
      "楽天市場・Yahoo!ショッピング・メルカリShopsなどECモールの商品カテゴリ・属性タグをAIで自動補完するSaaS。商品名・説明文・画像から最適なタグをAIが提案し、モールごとの仕様に合わせたCSVを出力。タグ付け工数を削減しながら、検索流入と販売機会を最大化します。",
    image: "/products/catamap.png",
    url: EXTERNAL_LINKS.catamap,
    status: "New",
    features: [
      { icon: Sparkles, label: "AIタグ自動提案" },
      { icon: ShoppingBag, label: "モール別最適化" },
      { icon: FileSpreadsheet, label: "CSV一括出力" },
    ],
