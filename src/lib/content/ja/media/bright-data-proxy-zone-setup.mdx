...
faq:
  ...
  - q: "Zone 作成後の認証文字列はどこで確認できますか？"
    a: "ダッシュボードの該当 Zone を開き、「Access parameters」または「Integrate」タブを選ぶと、Zone 名・ユーザー名 (`brd-customer-XXX-zone-YYY` 形式)・パスワード・プロキシホスト・ポート番号が表示されます。さらに Python / Node.js / curl 等の言語別サンプルコードと、IP Whitelist で接続する場合の Password 不要の文字列もそこで生成されます。本番のシークレット管理ツール (AWS Secrets Manager / HashiCorp Vault 等) に登録する際は、必ずこの画面からコピーしてください。"
