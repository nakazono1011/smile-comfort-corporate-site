...
1. **Web 検索**
   ...
   - `searchWeb("<Primary Keyword> 最新 口コミ 料金", {recencyDays: 120})` や`brave-search` mcp を駆使して直近 4 か月以内の情報を取得。
   - 上位 10 件を走査し、料金表・リリースノート・FAQ を確認。

2. **Playwright MCP（自動ブラウズ／スクショ）**

   - 当該キーワードに関連する公式サイトの最新情報、競合記事のコンテンツをチェック
   - 関連性の高いユーザーの理解を促進する画像を検索して入手（他の記事の画像と重複は NG）
